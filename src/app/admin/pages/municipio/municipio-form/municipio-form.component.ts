...
export class MunicipioFormComponent implements OnInit {
  ...
  salvar() {
    ...
    console.log('Formulário:', this.formMunicipio.value);
    console.log('Formulário válido:', this.formMunicipio.valid);
    ...
    if (this.formMunicipio.valid) {
      ...
      if (municipio.id == null) {
        this.municipioService.insert(modeloAtualizado).subscribe({
          next: (municipioService) => {
        ...
        this.municipioService.update(modeloAtualizado).subscribe({
          next: (municipioService) => {
            this.router.navigateByUrl('/municipio/list');
          },
          error: (err) => {
